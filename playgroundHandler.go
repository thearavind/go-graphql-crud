...
const graphcoolPlaygroundVersion = "1.5.6"

func graphqlPlaygroundHandler(w http.ResponseWriter, r *http.Request) {
	_template := template.New("Playground")
	_template, err := _template.Parse(graphcoolPlaygroundTemplate)
	if err != nil {
	...
	err = _template.ExecuteTemplate(w, "index", _playgroundData)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
